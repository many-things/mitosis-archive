...
func Test_FinishSignature(t *testing.T) {
	k, ctx, cdc, storeKey := testkeeper.MultisigKeeper(t)
	repo := state.NewKVChainSignResultRepo(cdc, ctx.KVStore(storeKey), chainID)
	signRepo := state.NewKVChainSignRepo(cdc, ctx.KVStore(storeKey), chainID)
	valAddr := testutils.GenValAddress(t)
	...
	require.Nil(
		t, signRepo.Save(
			&exported.Sign{
				...
				Participants:  []sdk.ValAddress{valAddr},
				MessageToSign: digest,
				Status:        exported.Sign_StatusExecute,
			},
		),
	)
	...
	// sign status changes
	currentSign, err := signRepo.Load(0)
	require.Nil(t, err)
	require.Equal(t, currentSign.Status, exported.Sign_StatusComplete)
}
